...
class SudokuGUI:
    def __init__(self, root):
        ...
        self.timer_label = tk.Label(self.root, text="Time: 00:00:00", bg='#f0f0f0', font=('Helvetica', 12))
    ...
    def update_timer(self):
        ...
        self.timer_label.config(text=f"Time: {int(hours):02}:{int(minutes):02}:{int(seconds):02}")
    ...
    def show_math_operation_popup(self, operation, result, row, col):
        def check_answer():
            try:
                user_answer = int(answeruser_answer = int(answer_entry.get()))
